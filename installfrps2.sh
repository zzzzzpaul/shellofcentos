...
sleep 3
clear
FRPPATH=/root/frp_0.35.1_darwin_amd64
echo "开始安装frp服务端, 脚本报错请先检查脚本, 并确定已安装docker, docker-compose, 并开放7000端口" &&\
	echo "下载frp包" &&\
	wget https://github.com/fatedier/frp/releases/download/v0.35.1/frp_0.35.1_darwin_amd64.tar.gz &&\
	echo "解压frp安装包" &&\
	tar -vxf frp_0.35.1_darwin_amd64.tar.gz &&\
	cd $FRPPATH &&\
